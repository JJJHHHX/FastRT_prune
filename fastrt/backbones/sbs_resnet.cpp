...
namespace fastrt {
    ...
    ILayer* backbone_sbsR50::topology(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input) {
        ...
        Dims out_dim = pool1->getOutput(0)->getDimensions();
        std::cout<<"test output size"<<" "<<out_dim.d[0]<<" "<<out_dim.d[1]<<" "<<out_dim.d[2]<<std::endl;
    ...
    ILayer* backbone_sbsR50_pruned::topology(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input) {
        /*
         * Reference: https://github.com/JDAI-CV/fast-reid/blob/master/fastreid/modeling/backbones/resnet.py
         * NL layers follow by: nl_layers_per_stage = {'50x': [0, 2, 3, 0],}[depth]
         * for nn.MaxPool2d(kernel_size=3, stride=2, ceil_mode=True) => pool1->setPaddingMode(PaddingMode::kEXPLICIT_ROUND_UP);
        ...
        // pruned conv output shape map
        std::map<std::string, int> ConvOutputShapeMap=loadPrunedConvOutputShape("/home/xujiahong/trt_engine/FastRT_pruned/pruned_conv_output_shape_0.5.txt");
        std::string ibn{""};
        if(_modelCfg.with_ibna) {
            ibn = "a";
        }
        ...
        IActivationLayer* x = bottleneck_ibn_pruned(network, weightMap, ConvOutputShapeMap, *pool1->getOutput(0), 64, 64, 1, "backbone.layer1.0.", ibn_layers[ibn][0]);
        x = bottleneck_ibn_pruned(network, weightMap, ConvOutputShapeMap, *x->getOutput(0), 256, 64, 1, "backbone.layer1.1.", ibn_layers[ibn][1]);
        x = bottleneck_ibn_pruned(network, weightMap, ConvOutputShapeMap, *x->getOutput(0), 256, 64, 1, "backbone.layer1.2.", ibn_layers[ibn][2]);

        x = bottleneck_ibn_pruned(network, weightMap, ConvOutputShapeMap, *x->getOutput(0), 256, 128, 2, "backbone.layer2.0.", ibn_layers[ibn][3]);
        x = bottleneck_ibn_pruned(network, weightMap, ConvOutputShapeMap, *x->getOutput(0), 512, 128, 1, "backbone.layer2.1.", ibn_layers[ibn][4]);
        x = bottleneck_ibn_pruned(network, weightMap, ConvOutputShapeMap, *x->getOutput(0), 512, 128, 1, "backbone.layer2.2.", ibn_layers[ibn][5]);
        ILayer* _layer{x};
        if(_modelCfg.with_nl) {
            _layer = Non_local(network, weightMap, *x->getOutput(0), "backbone.NL_2.0.");
        }
        x = bottleneck_ibn_pruned(network, weightMap, ConvOutputShapeMap, *_layer->getOutput(0), 512, 128, 1, "backbone.layer2.3.", ibn_layers[ibn][6]);
        _layer = x;
        if(_modelCfg.with_nl) {
            _layer = Non_local(network, weightMap, *x->getOutput(0), "backbone.NL_2.1.");
        }

        x = bottleneck_ibn_pruned(network, weightMap, ConvOutputShapeMap, *_layer->getOutput(0), 512, 256, 2, "backbone.layer3.0.", ibn_layers[ibn][7]);
        x = bottleneck_ibn_pruned(network, weightMap, ConvOutputShapeMap, *x->getOutput(0), 1024, 256, 1, "backbone.layer3.1.", ibn_layers[ibn][8]);
        x = bottleneck_ibn_pruned(network, weightMap, ConvOutputShapeMap, *x->getOutput(0), 1024, 256, 1, "backbone.layer3.2.", ibn_layers[ibn][9]);
        x = bottleneck_ibn_pruned(network, weightMap, ConvOutputShapeMap, *x->getOutput(0), 1024, 256, 1, "backbone.layer3.3.", ibn_layers[ibn][10]);
        _layer = x;
        if(_modelCfg.with_nl) {
            _layer = Non_local(network, weightMap, *x->getOutput(0), "backbone.NL_3.0.");
        } 
        x = bottleneck_ibn_pruned(network, weightMap, ConvOutputShapeMap, *_layer->getOutput(0), 1024, 256, 1, "backbone.layer3.4.", ibn_layers[ibn][11]);
        _layer = x;
        if(_modelCfg.with_nl) {
            _layer = Non_local(network, weightMap, *x->getOutput(0), "backbone.NL_3.1.");
        }
        x = bottleneck_ibn_pruned(network, weightMap, ConvOutputShapeMap, *_layer->getOutput(0), 1024, 256, 1, "backbone.layer3.5.", ibn_layers[ibn][12]);
        _layer = x;
        if(_modelCfg.with_nl) {
            _layer = Non_local(network, weightMap, *x->getOutput(0), "backbone.NL_3.2.");
        }

        x = bottleneck_ibn_pruned(network, weightMap, ConvOutputShapeMap, *_layer->getOutput(0), 1024, 512, _modelCfg.last_stride, "backbone.layer4.0.", ibn_layers[ibn][13]); 
        x = bottleneck_ibn_pruned(network, weightMap, ConvOutputShapeMap, *x->getOutput(0), 2048, 512, 1, "backbone.layer4.1.", ibn_layers[ibn][14]);
        x = bottleneck_ibn_pruned(network, weightMap, ConvOutputShapeMap, *x->getOutput(0), 2048, 512, 1, "backbone.layer4.2.", ibn_layers[ibn][15]);
        return x;
    }
