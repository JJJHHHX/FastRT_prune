...
namespace trtxapi {
    ...
    IActivationLayer* bottleneck_ibn_pruned(INetworkDefinition* network, std::map<std::string, Weights>& weightMap, std::map<std::string, int>& ConvOutputShapeMap, ITensor& input, const int inch, const int outch, const int stride, const std::string lname, const std::string ibn) {
        Weights emptywts{DataType::kFLOAT, nullptr, 0};
        IConvolutionLayer* conv1 = network->addConvolutionNd(input, ConvOutputShapeMap[lname + "conv1.weight"], DimsHW{1, 1}, weightMap[lname + "conv1.weight"], emptywts);
        TRTASSERT(conv1);
        ...
        IConvolutionLayer* conv2 = network->addConvolutionNd(*relu1->getOutput(0), ConvOutputShapeMap[lname + "conv2.weight"], DimsHW{3, 3}, weightMap[lname + "conv2.weight"], emptywts);
        TRTASSERT(conv2);
        conv2->setStrideNd(DimsHW{stride, stride});
        conv2->setPaddingNd(DimsHW{1, 1});
