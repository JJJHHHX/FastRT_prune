...
namespace fastrt {
    ...
    ILayer* embedding_head::topology(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input) {
        ...
        if (_modelCfg.with_norm){
            std::cout<< " add output feature normalization"<<std::endl; 
            IElementWiseLayer* L2norm = trtxapi::addL2Norm(network, *bottleneck->getOutput(0),1e-5); 
            TRTASSERT(L2norm);
            return L2norm; 
        }
