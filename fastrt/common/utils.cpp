...
namespace trt {
    ...
    std::map<std::string, int> loadPrunedConvOutputShape(const std::string file){
        std::cout << "[Loading pruned conv output shape]: " << file << std::endl;
        std::map<std::string, int> ConvOutputShapeMap;
        ...
        while(input>>name>>std::dec>>output_shape){
            ConvOutputShapeMap[name] = output_shape;
        }      
        return ConvOutputShapeMap;  
    }
...
namespace fastrt {
    ...
    std::ostream& operator<<(std::ostream& os, const FastreidConfig& fastreidCfg) {
        os << "\tbackbone: "            << BackboneTypetoString(fastreidCfg.backbone) << "\n\t"
            ...
            << "with_norm: "            << fastreidCfg.with_norm                      << "\n";
        return os;   
    } 
