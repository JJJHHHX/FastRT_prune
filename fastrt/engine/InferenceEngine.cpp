...
namespace trt {
   ...
   InferenceEngine::InferenceEngine(const EngineConfig &enginecfg): _engineCfg(enginecfg) { 
        ...
        // _outputSize = _engineCfg.max_batch_size * 64 * 64 * 32; 
    ...
    bool InferenceEngine::doInference(const int inference_batch_size, std::function<void(float*)> preprocessing) {
        ...
        std::cout<<"host to device time "<<t2-t1<<" s"<<std::endl;
        std::cout<<"cpu inference time "<<t3-t2<<" s"<<std::endl;
        std::cout<<"device to host time "<<t4-t3<<" s"<<std::endl;
        return status;
    }
